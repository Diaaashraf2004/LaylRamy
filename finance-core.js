...
// formatCurrency is defined in finance-shared.js (window.formatCurrency)
// Local fallback in case shared loads late:
function formatCurrency(amount) { return window.formatCurrency ? window.formatCurrency(amount) : (Number(amount) || 0) + ' ج.م'; }
...
function sanitizeDataForFirebase(obj) {
    ...
    return newObj;
        ...
        function updateLoadDateDayName(dateString, displayElement) { if (!displayElement) { /* console.warn("Day name display element not provided!"); */ return; } if (!dateString || typeof dateString !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(dateString)) { displayElement.textContent = ''; return; } try { const date = new Date(dateString + 'T00:00:00'); // Ensure it's treated as local
             const options = { timeZone: 'Africa/Cairo', weekday: 'long' }; const dayName = date.toLocaleDateString('ar-EG', options); displayElement.textContent = `(${escapeHTML(dayName)})`; } catch (e) { console.error("Error getting day name for", dateString, ":", e); displayElement.textContent = ''; } }
...
async function saveInvoiceToFirestore(invoiceRecord, returnBatchOp = false) {
   ...
   if (!invoiceRecord.saleDate) {
    ...
    if (returnBatchOp) {
        return { type: 'set', ref: invoiceDocRef, data: invoiceRecord, opts: { merge: true } };
    }

    try {
        await FinanceSync.safeSet(invoiceDocRef, invoiceRecord, { merge: true }, 'saveInvoiceToFirestore');
    ...
    return purchaseInvoices.reduce((total, invoice) => total + (Number(invoice.grandTotal) || 0), 0);
        ...
        function updateSuppliersListDisplay() {
             ...
             if (suppliers.length === 0) {
                 ...
                 supplierList.innerHTML = sortedSuppliers.map(supplier => `
                     <div class="bg-gray-50 rounded-lg shadow p-4 border border-gray-200 flex flex-col justify-between">
                         <div>
                             <h4 class="text-lg font-semibold text-gray-800 mb-2">${escapeHTML(supplier.name)}</h4>
                             ${supplier.contact ? `<p class="text-gray-600 text-sm">الاتصال: ${escapeHTML(supplier.contact)}</p>` : ''}
                             ${supplier.address ? `<p class="text-gray-600 text-sm">العنوان: ${escapeHTML(supplier.address)}</p>` : ''}
                         </div>
             ...
             if(supplierNamesDatalist) {
                 supplierNamesDatalist.innerHTML = suppliers.map(s => `<option value="${escapeHTML(s.name)}"></option>`).join('');
             }
         }
        ...
        function deleteSupplier(supplierId) {
             ...
             if (supplierIndex > -1) {
                 ...
                 if (confirm(`هل أنت متأكد من حذف المورد "${escapeHTML(supplierName)}"؟ سيتم إزالة ربطه من جميع المنتجات والأرقام التسلسلية المسجلة باسمه.`)) {
                     ...
                     logOperation("حذف مورد", `تم حذف المورد "${escapeHTML(supplierName)}". تم فك ارتباط ${updatedProductsCount} منتج. تم حذف ${deletedSerialCount} رقم تسلسلي مسجل باسمه.`);
                     showMessage(supplierMessage, `تم حذف المورد "${escapeHTML(supplierName)}" والسيريالات المرتبطة به.`);
...
function updateProductListDisplay() {
    ...
    if (categoryDatalist) {
        categoryDatalist.innerHTML = allCategories.map(cat => `<option value="${escapeHTML(cat)}"></option>`).join('');
    }
...
const filteredProducts = products.filter(product => {
    ...
    if (filteredProducts.length === 0) {
        ...
        const productsHTML = sortedProducts.map(product => {
            ...
            const categoryHTML = product.category ? `<p class="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full inline-block mt-2">${escapeHTML(product.category)}</p>` : '';
            ...
            return `<div class="bg-gray-50 rounded-lg shadow p-4 border border-gray-200 flex flex-col justify-between">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200 flex justify-end gap-2 product-actions">
                            <button data-product-id="${product.id || ''}" data-product-name="${escapeHTML(product.name)}" class="manage-serials-btn text-xs font-semibold py-1 px-2 rounded">إدارة السيريالات</button>
                         <button data-product-id="${product.id || ''}" data-product-name="${escapeHTML(product.name)}" class="edit-product-btn text-xs bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-semibold py-1 px-2 rounded border border-yellow-500">تعديل</button>
                           <button data-product-id="${product.id || ''}" data-product-name="${escapeHTML(product.name)}" class="delete-product-btn text-xs bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-2 rounded border border-red-600">حذف</button>
                        </div>
                    </div>`;
        }).join('');
  ...
  // الداتاليست لازم تفضل تبعت الاسم فقط (هي للعرض/الإكمال النصي، مش بحث بالـ ID)
    if (productNamesDatalist) {
        productNamesDatalist.innerHTML = products.map(p => `<option value="${escapeHTML(p.name)}">${escapeHTML(p.name)} (القسم: ${p.category || 'عام'}) - المتاح: ${p.quantity}</option>`).join('');
    }
}
    ...
    tableBody.innerHTML = filteredLogs.map((log, index) => {
        ...
        return `
            <tr class="hover:bg-slate-50/80 transition-colors">
                ...
                <td class="px-4 py-3.5 text-slate-700 font-semibold leading-relaxed">${escapeHTML(log.details)}</td>
...
function renderCollectionNotesHistory(customerId) {
    ...
    collectionNotesHistory.innerHTML = notes.map(note => `
        <div class="p-3 border rounded mb-2 bg-gray-50">
            <div><strong>الملاحظة:</strong> ${escapeHTML(note.note)}</div>
            ...
            <div>
                ...
                ${
                    ...
                    ? customerNotes.map(note => `
                        <div style="margin-top:8px;">
                            - ${escapeHTML(note.note)} | متابعة: ${note.nextFollowUpDate || 'لا يوجد'} | ${new Date(note.createdAt).toLocaleString('ar-EG')}
                        </div>
                    `).join("")
    ...
    creditorStatementContent.innerHTML = `
        <div class="space-y-4" id="print-creditor-statement-area">
            <div class="text-center mb-6 border-b pb-4">
                ...
                <p class="text-gray-600 mt-2"><strong>اسم المورد:</strong> ${escapeHTML(creditorName)}</p>
                        ...
                        <tbody>
                            ${allPayments.map(p => `
                                <tr>
                                    ...
                                    <td class="border p-2">${escapeHTML(p.accountName)}</td>
...
window.renameLiability = function(liabilityId) {
    ...
    if (newName && newName.trim() !== "" && newName.trim() !== liability.name) {
        ...
        logOperation("تعديل اسم التزام", `تم تعديل اسم الالتزام من "${escapeHTML(oldName)}" إلى "${escapeHTML(liability.name)}".`);
        updateUI();
    }
};
// -----------------------------
...
function updateDebtorsListDisplay() {
    ...
    customers.forEach(group => {
        ...
        topInfoLi.innerHTML = `
            ...
            ${
                latestNote
                ? `<div class="text-sm p-2 rounded bg-yellow-50 border border-yellow-200"><strong>آخر ملاحظة:</strong> ${escapeHTML(latestNote.note)}<br><strong>متابعة:</strong> ${latestNote.nextFollowUpDate || 'لا يوجد'}</div>`
            ...
            <div class="debt-pay-selected-bar mt-2" data-customer-id="${group.customerId}" style="display:none;">
                <div class="flex gap-2 items-center flex-wrap">
                    <select class="debt-pay-account-select text-xs py-1 px-2 border rounded flex-grow" data-customer-id="${group.customerId}" style="min-width:120px;">
                        ${accounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})</option>`).join('')}
                    </select>
...
function toggleInlinePaymentForm(debtorName) {
    console.log(`toggleInlinePaymentForm called for: ${escapeHTML(debtorName)}`);

    const targetGroup = document.querySelector(`.debtor-group[data-debtor-name="${escapeHTML(debtorName)}"]`);
    if (!targetGroup) {
    ...
    let accountOptions = accounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})</option>`).join('');

    formDiv.innerHTML = `
        <p class="text-sm font-semibold text-gray-700 mb-3">استلام دفعة من ${escapeHTML(debtorName)} (الإجمالي: ${formatCurrency(totalDebt)})</p>
        <div class="form-grid">
...
// =====================================================================        
    ...
    sortedLiabilities.forEach(liability => {
        ...
        let nameHTML = `<span class="font-medium ${isPaid ? 'text-gray-500 line-through' : ''}">${escapeHTML(liability.name)}</span>`; // الشكل الافتراضي للاسم
            ...
            nameHTML = `
                <div class="view-purchase-invoice-btn" style="cursor: pointer;" data-purchase-invoice-id="${liability.purchaseInvoiceId}">
                    <span class="font-medium text-blue-700 hover:underline ${isPaid ? 'line-through' : ''}">
                        <i class="fas fa-file-invoice ml-1"></i> ${escapeHTML(liability.name)}
                    </span>
        ...
        li.innerHTML = `
            <div class="flex-grow pr-4">
                ${nameHTML}
                ${liability.category && liability.category !== 'أخرى' ? `<span class="inline-block mt-1 text-[10px] bg-gray-200 text-gray-600 px-2 py-0.5 rounded">${escapeHTML(liability.category)}</span>` : ''}
...
function renderDebtorPaymentList() {
    ...
    container.innerHTML = sortedDebts.map(debt => `
        <label>
            ...
            <span class="flex-grow">${escapeHTML(debt.name)} - ${debt.reason || 'دين عام'}</span>
...
function renderLiabilityPaymentList() {
    ...
    container.innerHTML = sortedLiabilities.map(liability => `
        <label>
            ...
            <span class="flex-grow">${escapeHTML(liability.name)}</span>
...
function updatePendingSalesDisplay() {
    ...
    pendingSalesListContainer.innerHTML = filteredSales.map(sale => {
        ...
        if (attachments && attachments.length > 0) {
            attachmentsHTML = `<div class="text-xs text-gray-500 mt-1 pl-4" style="margin-right: 10px;"><strong>+ ملحقات:</strong> ${attachments.map(item => `${item.quantity}x ${escapeHTML(item.name)}`).join(', ')}</div>`;
        }
        
        // 2. عرض المنتجات
        if (sale.items) {
            itemsText = sale.items.map(item => `${item.quantity}x "${escapeHTML(item.name)}"`).join(', ');
            if (itemsText.length > 50) itemsText = itemsText.substring(0, 50) + "...";
            itemsText = `<b>[فاتورة]</b> ${itemsText}`;
        } else if (sale.mainProduct) {
            itemsText = `${sale.mainProduct.quantity}x "${escapeHTML(sale.mainProduct.name)}"`;
        }
        
        let customerInfo = sale.customerName ? `<span class="block text-sm text-blue-600">العميل: ${escapeHTML(sale.customerName)}</span>` : '';
 ...
 function updateOffsetDatalists() { if (!debtorNamesDatalistOffset || !creditorNamesDatalistOffset) return; // Populate Debtor Datalist
             ...
             debtorNamesDatalistOffset.innerHTML = debtorNames.map(name => `<option value="${escapeHTML(name)}"></option>`).join('');
             ...
             creditorNamesDatalistOffset.innerHTML = creditorNames.map(name => `<option value="${escapeHTML(name)}"></option>`).join('');
          }
...
window.convertPendingSaleToDebt = async function(pendingSaleId, debtMode, targetDebtId, confirmSale, accountId) {
    ...
    if (debtMode === 'existing' && targetDebtId) {
        ...
        if (existingDebt) {
            ...
            showGlobalMessage(`تم إضافة بند دين مستقل لحساب: ${escapeHTML(targetCustomerName)}`);
            done = true;
        }
    }
    ...
    if (!done) {
        ...
        showGlobalMessage(`تم تسجيل دين لعميل جديد: ${escapeHTML(saleData.customerName)}`);
    }
...
function populateReturnProductSelect() {
    ...
    sortedProducts.forEach(p => {
        ...
        option.textContent = `${escapeHTML(p.name)} (القسم: ${p.category || 'عام'}) - المتاح: ${p.quantity}`;
...
function updateCompletedReturnsDisplay() {
    ...
    completedReturnsList.innerHTML = sortedReturns.map(ret => `
        <div class="p-2 border-b">
            <div><strong>${(ret.items || []).map(item => `${item.quantity}x ${escapeHTML(item.name)}`).join(', ')}</strong> <span class="text-sm text-gray-500">(${escapeHTML(ret.customerName)})</span></div>
    ...
    pendingReceiptList.innerHTML = sortedReturns.map(ret => `
        <div class="p-3 border rounded-md bg-yellow-50 shadow-sm mb-2">
            <div class="flex justify-between items-center">
                <div>
                    <strong>${(ret.items || []).map(item => `${item.quantity}x ${escapeHTML(item.name)}`).join(', ')}</strong> 
                    <span class="text-sm text-gray-500">(${escapeHTML(ret.customerName)})</span>
                    <div class="text-xs text-gray-400">${formatDateTime(ret.timestamp)}</div>
                </div>
   ...
   // قراءة البيانات من المدخلات بالـ ID أو الاسم لمنع المنتجات الوهمية
    ...
    if (financialType === 'cash') {
        if (returnedAmount > account.balance) { 
            showMessage(returnMessage, `رصيد حساب "${escapeHTML(account.name)}" غير كافٍ.`, true); 
            return; 
        }
        account.balance -= returnedAmount;
        logDetails = `إرجاع نقدي ${formatCurrency(returnedAmount)} للعميل "${escapeHTML(customerName)}" من حساب "${escapeHTML(account.name)}".`;
    } else if (financialType === 'credit') {
        liabilities.push({ id: generateId('liab'), name: `رصيد مرتجع للعميل: ${escapeHTML(customerName)}`, amount: returnedAmount });
        logDetails = `تسجيل رصيد للعميل "${escapeHTML(customerName)}" بقيمة ${formatCurrency(returnedAmount)}.`;
    }
    ...
    name: `${escapeHTML(productName)} - ${escapeHTML(customerName)} (مرتجع)`, 
    quantity: quantity, 
        ...
        if (docSnap.exists()) {
            ...
            if (dataToMigrate) {
                ...
                dataToMigrate.log = [{
                    ...
                    timestamp: new Date().toISOString(),
                    type: "ترحيل تلقائي",
                    details: `بدء يوم جديد بترحيل الأصول من: ${escapeHTML(sourceName)}`
                }];
                ...
                dataToMigrate.liquidityLog = [{
                    ...
                    type: "adjust", 
                    amount: 0,
                    description: `رصيد افتتاح مرحل من ${escapeHTML(sourceName)}`,
...
function updateMonthlyLiabilitiesDisplay() {
    ...
    sortedLiabilities.forEach(liability => {
        ...
        li.innerHTML = `
            <div>
                <span class="font-semibold text-gray-800">${escapeHTML(liability.name)}</span>
...
function addOrUpdateMonthlyLiability() {
    ...
    if (editingMonthlyLiabilityId) {
        ...
        if (liability) {
            ...
            logOperation("تعديل التزام شهري", `تم تعديل الالتزام الشهري "${escapeHTML(name)}" إلى مبلغ ${formatCurrency(amount)}.`);
        ...
        if (monthlyLiabilities.some(l => l.name.toLowerCase() === name.toLowerCase())) {
             showMessage(monthlyLiabilityMessage, `الالتزام الشهري بالاسم "${escapeHTML(name)}" موجود بالفعل.`, true);
             return;
        }
        const newLiability = { id: generateId('m-liab'), name, amount };
        monthlyLiabilities.push(newLiability);
        logOperation("إضافة التزام شهري", `تمت إضافة التزام شهري جديد: "${escapeHTML(name)}" بقيمة ${formatCurrency(amount)}.`);
...
function processMonthlyLiabilities() {
    ...
    monthlyLiabilities.forEach(mLiability => {
        const liabilityName = `التزام شهري: ${escapeHTML(mLiability.name)}`;
        const newLiability = {
            id: generateId('liab'),
            name: liabilityName,
        ...
        addedItemsLog.push(`"${escapeHTML(mLiability.name)}" بقيمة ${formatCurrency(mLiability.amount)}`);
    });
...
// =======================================================
    ...
    // 3. تأكيد من المستخدم
    if (confirm(`هل أنت متأكد من تغيير رصيد حساب "${escapeHTML(account.name)}" من ${formatCurrency(oldBalance)} إلى ${formatCurrency(newBalance)}؟`)) {
        ...
        logOperation("تعديل رصيد يدوي", `تم تعديل رصيد حساب "${escapeHTML(account.name)}" إلى ${formatCurrency(newBalance)}. السبب: ${escapeHTML(reason)}.`);
        ...
        liquidityLog.push({
            ...
            type: "adjust",
            amount: adjustment,
            description: `تعديل يدوي لحساب "${escapeHTML(account.name)}" - ${escapeHTML(reason)}`,
            currentBalance: newTotalLiquidity 
        });

        // 6. تحديث الواجهة وتنظيف النموذج
        updateUI();
        showMessage(messageEl, `تم تعديل رصيد حساب "${escapeHTML(account.name)}" بنجاح.`, false);
...
if (available < qtyToDeduct) {
                 ...
                 if (tempUnavailableDeducted) {
                     isValid = false;
                     errors.push(`الكمية المطلوبة للخصم (${tempUnavailableDeducted.required}) من "${escapeHTML(tempUnavailableDeducted.name)}" غير متوفرة (${tempUnavailableDeducted.available}).`);
                 }
...
function updateGlobalState(account, amountToCollect, customerName, grandTotal, depositPaid, accountId, items, isFromPending, pendingOrigin) {
    ...
    if (amountToCollect > 0 && account) {
        ...
        let logDesc = `فاتورة مبيعات ${escapeHTML(customerName)}`;
        if (depositPaid > 0) {
            logDesc = `استلام باقي فاتورة ${escapeHTML(customerName)} (إجمالي: ${grandTotal} - عربون: ${depositPaid})`;
        }
...
async function handleSaveInvoice(skipConfirmation = true) {
    ...
    if (isPending) {
        ...
        logOperation("فاتورة مؤقتة", `تم حفظ فاتورة مفصلة مؤقتة للعميل ${escapeHTML(customerName)} بقيمة ${formatCurrency(grandTotal)}.`);
        ...
        if (amountToCollectNow > 0) {
            ...
            if (depositAlreadyPaid > 0) {
                logDesc = `استلام باقي فاتورة ${escapeHTML(customerName)} (إجمالي: ${grandTotal} - عربون: ${depositAlreadyPaid})`;
            }
        ...
        const op = await saveInvoiceToFirestore(invoiceRecord, true);
        if (op) {
            await saveSystemToCloud([op]); 
        } else {
            await saveSystemToCloud();
        } 


        logOperation("إنشاء فاتورة", `فاتورة ${generatedInvoiceNumber} للعميل ${escapeHTML(customerName)}. (إجمالي: ${grandTotal})`);
    ...
    availableForDeduction.sort((a, b) => a.name.localeCompare(b.name, 'ar')).forEach(product => {
       ...
       ${isChecked}>
            <span style="flex-grow: 1; font-size: 0.9em;">
                 ${escapeHTML(product.name)} <span class="text-xs text-blue-500">(${product.category || 'عام'})</span>
...
function saveAddedSerialsFromModal() {
    ...
    updateUI(); // تحديث الواجهة الرئيسية لعكس التغييرات
            ...
            function performTwoPartyOffset() {
                ...
                logOperation("مقاصة", `تنفيذ مقاصة بمبلغ <span class="math-inline">\{formatCurrency\(offsetAmount\)\} بين المدين "</span>{debtorName}" والدائن "${escapeHTML(creditorName)}".`);
        ...
        filteredSales.forEach(sale => {
            ...
            let itemsDesc = (sale.items || []).map(item => `
                <div style="font-size: 0.85rem; margin-bottom: 2px;">
                    <span style="font-weight:bold; color:#334155;">${item.quantity}x</span> ${escapeHTML(item.name)}
                ...
                if (sale.items) {
                    ...
                    if (sale.additionalItems && sale.additionalItems.length > 0) {
                        sale.additionalItems.forEach(item => {
                            ...
                            addRow.innerHTML = `
                                <td class="item-name">${escapeHTML(item.name)} <span style="font-size:0.8em; color:grey;">(مرفق/حزمة)</span></td>
                                <td class="item-qty" style="text-align:center;">
        ...
        if (refundModal) {
            ...
            if (select) {
                select.innerHTML = accounts.map(acc => 
                    `<option value="${acc.id}">${escapeHTML(acc.name)} (رصيده الحالي: ${formatCurrency(acc.balance)})</option>`
                ).join('');
            }
     ...
     if (window.currentUser) {
         ...
         alert("يرجى تسجيل الدخول لعرض تفاصيل الفواتير المؤرشفة.");
            ...
            function searchSerialsBySupplier() {
                ...
                if (!supplier) {
                    showMessage(serialSearchMessage, `لم يتم العثور على مورد بالاسم "${escapeHTML(supplierName)}".`, true);
                    supplierSerialsResultsContainer.innerHTML = `<p class="text-center text-gray-500">لم يتم العثور على مورد بالاسم "${escapeHTML(supplierName)}".</p>`;
                    return;
                }
                ...
                if (foundSerials.length === 0) {
                    showMessage(serialSearchMessage, `لا توجد أرقام تسلسلية مسجلة للمورد "${escapeHTML(supplierName)}".`, false, true); // Info message
            ...
            function displayFilteredSupplierSerials(searchTerm = '') {
                 ...
                 if (filteredList.length === 0) {
                      supplierSerialsResultsContainer.innerHTML = `<p class="text-center text-gray-500">لا توجد أرقام تسلسلية تطابق "${escapeHTML(searchTerm)}".</p>`;
                      return;
                 }
        ...
        monthlyLiabilities.forEach(liability => {
            if (liability.isAmortizedDebt) {
                ...
                if (liability.periodsProcessed < liability.totalPeriods) {
                    monthlyExpenses.set(liability.id, { name: `${escapeHTML(liability.name)} [شهر ${liability.periodsProcessed + 1}/${liability.totalPeriods}]`, amount: liability.amount });
                }
            } else {
...
// Populate the UI lists for the selected month with REAL data
 async function fc_populate_lists(month) {
    ...
    fc_expenses_list.innerHTML = expenses.length > 0
        ? expenses.map(([id, exp]) => `<label class="flex items-center p-2 rounded-md hover:bg-gray-100 cursor-pointer"><input type="checkbox" data-id="${id}" data-amount="${exp.amount}" class="fc-expense-checkbox ml-3" checked>${escapeHTML(exp.name)} <span class="mr-auto font-mono text-gray-600">${formatCurrency(exp.amount)}</span></label>`).join('')
...
function fc_populate_debt_treatment() {
    ...
    fc_debt_select.innerHTML = '<option value="">-- اختر الدين للمعالجة --</option>' 
        + validDebts.map(d => `<option value="${d.id}" data-amount="${d.amount}">${escapeHTML(d.name)} - ${escapeHTML(d.reason)} (${formatCurrency(d.amount)})</option>`).join('');
}
...
async function fc_populate_month_select() {
    ...
    try {
        ...
        fc_month_select.innerHTML = sortedMonths.map(month => {
            ...
            return `<option value="${month}">${escapeHTML(monthName)}</option>`;
        }).join('');
...
function fc_populate_manual_inputs() {
    ...
    fc_manual_list.innerHTML = selectedProducts.map(prod => `
        <div class="flex items-center justify-between p-2">
            <label for="fc-perc-${prod.dataset.id}">${escapeHTML(prod.dataset.name)}</label>
            <div class="flex items-center">
                <input type="number" 
                       id="fc-perc-${prod.dataset.id}" 
                       data-product-id="${prod.dataset.id}" 
                       data-product-name="${escapeHTML(prod.dataset.name)}" 
...
function fc_execute_cost_distribution() {
    ...
    distributedExpenseCheckboxes.forEach(checkbox => {
        ...
        if (liabilityToUpdate) {
            ...
            logOperation("تحديث معالجة دين", `تم توزيع القسط ${liabilityToUpdate.periodsProcessed} من ${liabilityToUpdate.totalPeriods} للدين "${escapeHTML(liabilityToUpdate.name)}".`);
        }
    });
...
function fc_execute_debt_treatment() {
    ...
    if (!confirm(`هل أنت متأكد من تحويل دين "${escapeHTML(debtInfo.name)}" بقيمة ${formatCurrency(debtAmount)} إلى التزام شهري؟\nسيتم حذف الدين الحالي وإنشاء التزام شهري بقيمة ${formatCurrency(costPerMonth)} لمدة ${periodValue} شهور.`)) {
        return;
    }
    ...
    const newMonthlyLiability = {
        ...
        name: `معالجة دين (${escapeHTML(debtInfo.name)})`,
        amount: costPerMonth,
    ...
    logOperation("معالجة دين متعثر", `تم تحويل دين "${escapeHTML(debtInfo.name)}" (${formatCurrency(debtAmount)}) إلى التزام شهري ثابت بقيمة ${formatCurrency(costPerMonth)} لمدة ${periodValue} شهور.`);
...
function handleConfirmReceipt(pendingId) {
    ...
    ret.items.forEach(item => {
        if (ret.returnAtSalePrice) {
            const newProductName = `${escapeHTML(item.name)} - ${escapeHTML(ret.customerName)} (مرتجع)`;
            const uniqueProductCode = "code_" + Date.now() + "_" + Math.floor(Math.random() * 1000);
...
function pi_addItem() {
    ...
    newRow.innerHTML = `
        <td class="border p-2">
            ${escapeHTML(name)} 
            ${category ? `<span class="text-xs text-blue-600 block">(${escapeHTML(category)})</span>` : ''} 
...
function pi_validateForm() {
    ...
    if (deductFromLiquidity && paidAmount > 0) {
        ...
        if (!account) {
            ...
            errors.push(`رصيد حساب "${escapeHTML(account.name)}" (${formatCurrency(account.balance)}) غير كافٍ لتغطية المبلغ المدفوع (${formatCurrency(paidAmount)}).`);
        }
    }
        ...
        for (let i = 0; i < invoiceData.items.length; i++) {
            ...
            if (!item.duplicateResolution) {
                ...
                if (duplicate) {
                    ...
                    if (modal && modalText) {
                        modalText.innerHTML = `تنبيه (فاتورة المشتريات): البند <strong>"${escapeHTML(item.name)}"</strong> مسجل مسبقاً في المخزن بقسم <strong>"${duplicate.category || 'عام'}"</strong>.<br><br>الكمية الحالية له بالسيستم: <strong>${duplicate.quantity}</strong> قطعة.<br>اختر الإجراء المناسب لتوجيه هذا البند بالفاتورة:`;
            ...
            if (cost.linkedLiabilityId) {
                ...
                if (existingLiability) {
                    ...
                    logOperation(
                        ...
                        `تمت إضافة "${escapeHTML(cost.name)}" بقيمة ${formatCurrency(cost.amount)} إلى الالتزام "${escapeHTML(existingLiability.name)}" ضمن فاتورة الشراء رقم "${invoiceData.invoiceNumber || invoiceData.id}".`
                    );
                } else {
                    const newLiability = {
                        id: generateId('liab'),
                        name: `${escapeHTML(cost.name)} - ${supplier?.name || 'مورد غير محدد'}`,
                        amount: cost.amount,
                    ...
                    logOperation(
                        "إنشاء التزام تلقائي",
                        `تم إنشاء التزام جديد للتكلفة الإضافية "${escapeHTML(cost.name)}" بقيمة ${formatCurrency(cost.amount)} لأن الالتزام المرتبط لم يتم العثور عليه.`
                    );
                }
            } else {
                const newLiability = {
                ...
                logOperation(
                    ...
                    `تم إنشاء التزام جديد باسم "${escapeHTML(newLiability.name)}" بقيمة ${formatCurrency(cost.amount)} من فاتورة الشراء رقم "${invoiceData.invoiceNumber || invoiceData.id}".`
                );
            }
        });
...
window.pi_addExtraCostRow = function() {
    ...
    if (typeof liabilities !== 'undefined' && Array.isArray(liabilities)) {
        liabilities.filter(l => parseFloat(l.amount) > 0).forEach(l => {
            liabilitiesOptions += `<option value="${l.id}">${escapeHTML(l.name)} (مدين بـ: ${l.amount})</option>`;
        });
    }
...
function renderAccounts() {
    ...
    accounts.forEach(acc => {
        ...
        safeCard.innerHTML = `<h4><i class="fas fa-cash-register"></i> ${escapeHTML(acc.name)}</h4><div class="safe-card-balance">${formatCurrency(acc.balance)}</div>`;
        safesGrid.appendChild(safeCard);

        const listItem = document.createElement('li');
        listItem.className = 'account-item';
        listItem.innerHTML = `<span class="account-item-name">${escapeHTML(acc.name)}</span><div class="account-item-actions"><button class="edit-btn" data-id="${acc.id}" style="background-color: #f59e0b;">تعديل</button><button class="delete-btn" data-id="${acc.id}" style="background-color: #ef4444;">حذف</button></div>`;
...
function handleIncomeAddition() {
    ...
    logOperation("إضافة إيراد", `إضافة ${formatCurrency(amount)} إلى حساب "${escapeHTML(account.name)}" تحت فئة "${escapeHTML(category)}".`);
...
function handleExpenseAddition() {
    ...
    if (amount > account.balance) {
        alert(`لا يوجد رصيد كافٍ في حساب "${escapeHTML(account.name)}". الرصيد المتاح: ${formatCurrency(account.balance)}.`);
        return false;
    }
    ...
    liquidityLog.push({
        ...
        amount: amount,
        description: `صرف مصروف (${escapeHTML(category)}) من حساب "${escapeHTML(account.name)}"`,
        currentBalance: newTotalLiquidity,
        accountId: accountId
    });
    ...
    logOperation("تسجيل مصروف", `بقيمة ${formatCurrency(amount)} - صرف من حساب "${escapeHTML(account.name)}" (فئة: ${escapeHTML(category)}).`);
...
function handleTransfer() {
    ...
    if (amount > fromAccount.balance) {
        alert(`لا يوجد رصيد كافٍ في حساب "${escapeHTML(fromAccount.name)}" لإتمام التحويل.`);
        return false;
    }
    ...
    logOperation("تحويل بين الحسابات", `تم تحويل ${formatCurrency(amount)} من "${escapeHTML(fromAccount.name)}" إلى "${escapeHTML(toAccount.name)}".`);
    return true;
}
...
function handleBalanceAdjustment() {
    ...
    if (confirm(`هل أنت متأكد من تغيير رصيد حساب "${escapeHTML(account.name)}" إلى ${formatCurrency(newBalance)}؟`)) {
        saveStateToHistory();
        account.balance = newBalance;
        logOperation("تعديل رصيد يدوي", `تم تعديل رصيد حساب "${escapeHTML(account.name)}" إلى ${formatCurrency(newBalance)}. السبب: ${escapeHTML(reason)}.`);
        return true;
...
if (piItemTotalCostInput) {
 ...
 // =======================================================
    ...
    window.openDepositModal = function(pendingId) {
        ...
        if (select) {
            ...
            accounts.forEach(acc => {
                ...
                op.value = acc.id;
                op.textContent = `${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})`;
    ...
    liquidityLog.push({
        ...
        type: "add",
        amount: amount,
        description: `عربون من: ${pendingSales[saleIndex].customerName} إلى "${escapeHTML(account.name)}"`,
        currentBalance: totalLiquidity,
        accountId: account.id
    });
    ...
    const msgBox = document.getElementById('global-message');
    if(msgBox) {
        showGlobalMessage(`تم استلام عربون ${formatCurrency(amount)} بنجاح في ${escapeHTML(account.name)}.`, false);
    } else {
            ...
            try {
                ...
                if (qty === 1 && itemName) {
                    ...
                    if (datalist && typeof serialNumbersLog !== 'undefined') {
                        ...
                        datalist.innerHTML = availableSerials.map(s => `<option value="${escapeHTML(s.serial)}">`).join('');
                    }
                } else {
                    container.classList.add('hidden');
    ...
    monthlyLiabilitiesList.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('edit-monthly-liability-btn')) {
            ...
            if (liability) {
                ...
                monthlyLiabilityFormTitle.textContent = `تعديل: ${escapeHTML(liability.name)}`;
            ...
            if (liabilityIndex > -1) {
                 ...
                 if (confirm(`هل أنت متأكد من حذف الالتزام الشهري "${escapeHTML(liabilityName)}"؟`)) {

                     monthlyLiabilities.splice(liabilityIndex, 1);

                     logOperation("حذف التزام شهري", `تم حذف الالتزام الشهري الدائم "${escapeHTML(liabilityName)}".`);

                     showMessage(monthlyLiabilityMessage, `تم حذف "${escapeHTML(liabilityName)}".`);
...
if (addProductButton) {
    addProductButton.addEventListener("click", () => {
        ...
        if (duplicateProduct && !window.bypassDuplicateCheck) {
            ...
            const modalText = document.getElementById("duplicateModalText");
            if (modal && modalText) {
                modalText.innerHTML = `تنبيه: يوجد بالفعل منتج مسجل بنفس الاسم <strong>"${escapeHTML(duplicateProduct.name)}"</strong> في قسم <strong>"${duplicateProduct.category || 'عام'}"</strong>.<br><br>الكمية الحالية له: <strong>${duplicateProduct.quantity}</strong> قطعة بمتوسط تكلفة <strong>${formatCurrency(duplicateProduct.costPrice)}</strong>.`;
                modal.style.display = "flex";
                document.querySelector('input[name="duplicateAction"][value="merge"]').checked = true;
                document.getElementById("renameInputContainer").classList.add('hidden');
        ...
        if (editingProductName || window.currentEditingProductID) {
            ...
            if (quantity < oldQty) {
                ...
                if (modal && modalText && accSelect) {
                    modalText.innerHTML = `لقد قمت بتقليص كمية منتج <strong>"${escapeHTML(oldProduct.name)}"</strong> من <strong>${oldQty}</strong> إلى <strong>${quantity}</strong> قطعة.<br><br>فارق الكمية المنقوصة هو <strong>${reducedQty}</strong> قطعة، وقيمتها المالية الإجمالية تساوي <strong>${formatCurrency(totalRefundValue)}</strong>.`;
                    
                    // شحن الخزن المتاحة بالبرنامج
                    accSelect.innerHTML = accounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (الرصيد الحالي: ${formatCurrency(acc.balance)})</option>`).join('');
            ...
            if (quantity > oldQty) {
                ...
                if (deductLiquidity) {
                    ...
                    if (costToDeduct > account.balance) {
                        showMessage(productMessage, `لا توجد سيولة كافية في حساب "${escapeHTML(account.name)}".`, true);
                        return;
                    }
                    account.balance -= costToDeduct;
                    logOperation("شراء بضاعة (تعديل كمية)", `خصم ${formatCurrency(costToDeduct)} من حساب "${escapeHTML(account.name)}" لزيادة كمية منتج "${escapeHTML(name)}" بمقدار ${addedQty} قطعة.`);
            ...
            showMessage(productMessage, `تم تحديث المنتج "${escapeHTML(name)}" بنجاح.`);
            resetProductForm();
        ...
        if (existingIndex === -1) {
            ...
            if (existingIndex !== -1) {
                 ...
                 if (deductLiquidity && newQuantity > 0) {
                    ...
                    account.balance -= costToDeduct;
                    logOperation("شراء بضاعة", `خصم ${formatCurrency(costToDeduct)} من حساب "${escapeHTML(account.name)}" لشراء ${newQuantity}x ${escapeHTML(name)}.`);
                }
                ...
                showMessage(productMessage, `تم تحديث كمية المنتج "${escapeHTML(name)}" بنجاح.`);
                ...
                if (deductLiquidity && newQuantity > 0) {
                    ...
                    account.balance -= costToDeduct;
                    logOperation("شراء بضاعة جديدة", `خصم ${formatCurrency(costToDeduct)} من حساب "${escapeHTML(account.name)}" لشراء ${newQuantity}x ${escapeHTML(name)}.`);
                }
...
if (confirmDecreaseBtn) {
    confirmDecreaseBtn.addEventListener("click", () => {
        ...
        if (actionType === 'deposit') {
            ...
            logOperation("تسوية مخزن (إيداع)", `إضافة ${formatCurrency(totalRefundValue)} لحساب "${escapeHTML(account.name)}" مقابل مرتجع/تقليص كمية منتج "${escapeHTML(name)}" بمقدار ${reducedQty} قطعة.`);
        } else {
            // الخيار الثاني: الخصم المباشر من رأس المال (تسجيل خسارة / هالك بضاعة) دون كاش
            logOperation("تسوية مخزن (هالك/خسارة)", `تقليص كمية منتج "${escapeHTML(name)}" بمقدار ${reducedQty} قطعة واعتبارها هالك/خسارة قيمتها ${formatCurrency(totalRefundValue)} دون إدخال سيولة.`);
        }
        ...
        showMessage(productMessage, `تم تسوية الفارق الحسابي وتحديث المنتج "${escapeHTML(name)}" بنجاح.`);
        closeDecreaseModal();
    ...
    confirmDuplicateBtn.addEventListener("click", () => {
        ...
        if (window.pendingDuplicateProduct) {
            ...
            if (selectedAction === 'merge') {
                ...
                if (data.deductLiquidity && data.quantity > 0 && account) {
                    ...
                    if (costToDeduct <= account.balance) {
                        account.balance -= costToDeduct;
                        logOperation("شراء بضاعة (منفصلة الاسم)", `خصم ${formatCurrency(costToDeduct)} من حساب "${escapeHTML(account.name)}" لشراء منتج مستقل يحمل اسم مكرر "${escapeHTML(data.name)}".`);
                    }
                }
                ...
                showMessage(productMessage, `تم تسجيل "${escapeHTML(data.name)}" كمنتج مستقل بكود جديد بنجاح.`);
                resetProductForm();
                updateUI();
            }
        }
        ...
        // [الحالة الثانية]: بند داخل فاتورة المشتريات
            ...
            if (productToEdit && productNameInput && productQuantityInput && productCostPriceInput && productSupplierSelect && addProductButton && productFormTitle && cancelEditProductButton) {
                ...
                productFormTitle.textContent = `تعديل المنتج: ${escapeHTML(productToEdit.name)}`;
                ...
                showMessage(productMessage, `لم يتم العثور على المنتج "${escapeHTML(productName)}" للتعديل أو عناصر النموذج غير جاهزة.`, true);
            }
        } else if (target.classList.contains('delete-product-btn')) {
            const productId = target.dataset.productId;
            const productName = target.dataset.productName;

            if (confirm(`هل أنت متأكد من حذف المنتج "${escapeHTML(productName)}" نهائياً من المخزن؟`)) {
                saveStateToHistory();
                ...
                if (productIndex !== -1) {
                    ...
                    logOperation("حذف منتج", `تم حذف المنتج "${escapeHTML(productName)}" (الكمية: ${qty} قطعة، التكلفة الإجمالية المحذوفة من رأس المال: ${formatCurrency(totalCardValue)}).`);
                    showMessage(productMessage, `تم حذف المنتج "${escapeHTML(productName)}" و ${deletedSerialCount} رقم تسلسلي مرتبط بنجاح.`);
...
function updateQuickSaleSerialUI() {
    ...
    if (productName) {
        ...
        sellSerialsDatalist.innerHTML = availableSerials.map(s => `<option value="${escapeHTML(s.serial)}">`).join('');
    } else {
...
function sellProduct(skipConfirmation = true) {
    ...
    if (mainProductIndex === -1) {
        showMessage(sellMessage, `المنتج "${escapeHTML(mainProductName)}" غير موجود في المخزن.`, true);
        return;
    }
    ...
    additionalCheckboxes.forEach(checkbox => {
        ...
        if (qtyToDeduct > 0 && additionalProductIndex !== -1) {
            ...
            if (additionalProduct.quantity < qtyToDeduct) {
                showMessage(sellMessage, `الكمية المطلوبة (${qtyToDeduct}) للمرفق "${escapeHTML(additionalProductName)}" غير متوفرة.`, true);
    ...
    if (isPendingSale) {
        ...
        logOperation("بيع مؤقت", `بيع مؤقت: ${escapeHTML(productNameWithSerial)}`);
        ...
        const newTotalLiquidity = accounts.reduce((sum, acc) => sum + acc.balance, 0);
        liquidityLog.push({ id: `liq-${Date.now()}`, timestamp: new Date().toISOString(), type: "add", amount: totalSellPrice, description: `بيع بضاعة: ${escapeHTML(productNameWithSerial)}`, currentBalance: newTotalLiquidity });
        ...
        logOperation("بيع بضاعة", `بيع ${escapeHTML(productNameWithSerial)} بسعر ${formatCurrency(totalSellPrice)}`);
...
if (addExpenseButton) {
    addExpenseButton.addEventListener("click", () => {
        ...
        if (amount > account.balance) {
            showMessage(expensesMessage, `مبلغ المصروف (${formatCurrency(amount)}) أكبر من رصيد حساب "${escapeHTML(account.name)}" (${formatCurrency(account.balance)}).`, true);
            return;
        }
        ...
        liquidityLog.push({ 
            ...
            type: "remove", 
            amount: amount, 
            description: `تسجيل مصروف من حساب "${escapeHTML(account.name)}"`, 
            currentBalance: newTotalLiquidity,
            accountId: selectedAccountId
        });
        
        logOperation("تسجيل مصروف", `تسجيل مصروف بقيمة ${formatCurrency(amount)}. تم خصمه من حساب "${escapeHTML(account.name)}".`);
...
if (removeExpenseButton) {
    removeExpenseButton.addEventListener("click", () => {
        ...
        liquidityLog.push({ 
            ...
            type: "add", 
            amount: amount, 
            description: `إلغاء/تقليل مصروف (استرداد) إلى حساب "${escapeHTML(account.name)}"`, 
            currentBalance: newTotalLiquidity,
            accountId: selectedAccountId
        });

        logOperation("تقليل مصروف", `تقليل المصروفات بقيمة ${formatCurrency(amount)}. تم إرجاعه لحساب "${escapeHTML(account.name)}".`);
...
if (addDebtButton) {
    addDebtButton.addEventListener("click", async () => {
        ...
        if (deductLiquidity) {
            ...
            if (amount > account.balance) {
                showMessage(debtsMessage, `مبلغ السلفة (${formatCurrency(amount)}) أكبر من رصيد حساب "${escapeHTML(account.name)}" (${formatCurrency(account.balance)}).`, true);
                return;
            }
        }
...
if (existingDebtIndex !== -1) {
    ...
    logMsg = `زيادة دين قائم على "${escapeHTML(customerProfile.name)}" بمبلغ ${formatCurrency(amount)}.`;
    showMessage(debtsMessage, `تمت زيادة الدين لـ "${escapeHTML(customerProfile.name)}".`);
} else {
    debtors.push({
        id: generateId('debt'),
    ...
    logMsg = `تسجيل دين جديد على "${escapeHTML(customerProfile.name)}" بمبلغ ${formatCurrency(amount)}.`;
    showMessage(debtsMessage, `تم تسجيل دين جديد لـ "${escapeHTML(customerProfile.name)}".`);
}
        ...
        if (deductLiquidity) {
            account.balance -= amount;
            const newTotalLiquidity = accounts.reduce((sum, acc) => sum + acc.balance, 0);
            liquidityLog.push({ id: `liq-${Date.now()}`, timestamp: new Date().toISOString(), type: "remove", amount: amount, description: `سلفة لـ ${escapeHTML(name)} من حساب "${escapeHTML(account.name)}"`, currentBalance: newTotalLiquidity });
            logMsg += ` وتم خصمه كسلفة من حساب "${escapeHTML(account.name)}".`;
        }
...
if (receivePaymentButton) {
    receivePaymentButton.addEventListener("click", () => {
        ...
        const summaryText = Array.from(paidDebtorsSummary.entries()).map(([name, amount]) => `${escapeHTML(name)} (${formatCurrency(amount)})`).join('، ');
        const newTotalLiquidity = accounts.reduce((sum, acc) => sum + acc.balance, 0);
        ...
        logOperation("استلام سداد مجمع", `استلام ${formatCurrency(totalPayment)} في حساب "${escapeHTML(account.name)}" من: ${summaryText}.`);
...
if (addLiabilityButton) {
    addLiabilityButton.addEventListener("click", async () => {
        ...
        if (existingIndex !== -1) {
            ...
            logMsg = `زيادة التزام قائم لـ "${escapeHTML(name)}" بمبلغ ${formatCurrency(amount)}. الإجمالي: ${formatCurrency(liabilities[existingIndex].amount)}`;
            showMessage(liabilitiesMessage, `تمت زيادة الالتزام لـ "${escapeHTML(name)}".`);
        } else {
            ...
            liabilities.push(newLiability);
            logMsg = `تسجيل التزام جديد على الشركة لـ "${escapeHTML(name)}" بمبلغ ${formatCurrency(amount)}.`;
            showMessage(liabilitiesMessage, `تم تسجيل التزام جديد لـ "${escapeHTML(name)}".`);
        }
        ...
        if (receivedCash) {
            ...
            const liqLogEntry = {
                ...
                type: "add",
                amount: amount,
                description: `استلام سيولة مقابل التزام لـ ${escapeHTML(name)} في حساب "${escapeHTML(account.name)}"`,
                currentBalance: newTotalLiquidity,
                accountId: selectedAccountId
            };
            liquidityLog.push(liqLogEntry);
            logMsg += ` وتم استلام سيولة مقابله في "${escapeHTML(account.name)}".`;
        }
...
// =======================================================
    ...
    payLiabilityButton.addEventListener("click", () => {
        ...
        if (totalPayment > account.balance) {
            showMessage(liabilitiesMessage, `رصيد حساب "${escapeHTML(account.name)}" (${formatCurrency(account.balance)}) غير كافٍ لتسديد المبلغ المطلوب (${formatCurrency(totalPayment)}).`, true);
            return;
        }
        ...
        const summaryText = Array.from(paidCreditorsSummary.entries()).map(([name, amount]) => `${escapeHTML(name)} (${formatCurrency(amount)})`).join('، ');
        const newTotalLiquidity = accounts.reduce((sum, acc) => sum + acc.balance, 0);
        const isExpense = d('batch-liability-is-expense') ? d('batch-liability-is-expense').checked : false;
        liquidityLog.push({ id: `liq-${Date.now()}`, timestamp: new Date().toISOString(), type: "out", amount: totalPayment, description: `تسديد التزام مجمع لـ: ${summaryText}`, currentBalance: newTotalLiquidity, accountId: account.id, isExpense: isExpense });
        
        if (isExpense) {
            expenses += totalPayment;
            logOperation("تسجيل مصروف", `بقيمة ${formatCurrency(totalPayment)} جنيه - سداد التزام مجمع لـ: ${summaryText}. (حساب: "${escapeHTML(account.name)}")`);
        } else {
            logOperation("تسديد التزام مجمع", `تسديد ${formatCurrency(totalPayment)} من حساب "${escapeHTML(account.name)}" إلى: ${summaryText}.`);
        }
...
if (ppConfirmBtn) {
    ppConfirmBtn.addEventListener('click', async () => {
        ...
        if (purchase.remainingBalance > account.balance) {
            showMessage(d('pp-payment-message'), `رصيد حساب "${escapeHTML(account.name)}" غير كافٍ.`, true);
            return;
        }
...
products.push({ 
    ...
    category: item.category || "" 
        ...
        liquidityLog.push({ 
            ...
            description: `تسديد متبقي مشتريات شاملة المصاريف من حساب "${escapeHTML(account.name)}"`, 
            currentBalance: newTotalLiquidity,
            accountId: account.id
        });
        ...
        logOperation("استلام وتسديد شامل", `تم استلام بضاعة وتسديد المتبقي شامل المصاريف من حساب "${escapeHTML(account.name)}".`);
                     ...
                     inv_itemQtyInput.addEventListener('input', () => {
                         ...
                         if (qty === 1 && itemName && products.some(p => p.name === itemName) && productHasSerialsRegistered) {
                             ...
                             if (!hasSerials && !inv_serialSearchInput.value) { // Show error only if no serials exist at all (and no search term)
                                 showMessage(inv_itemAddErrorDiv, `لا توجد أرقام تسلسلية متاحة في المخزون للمنتج "${escapeHTML(itemName)}".`, true);
    ...
    accountForm.addEventListener('submit', (e) => {
        ...
        if (id) { // Editing existing account
            ...
            if(account) {
                ...
                logOperation("تعديل حساب", `تم تغيير اسم حساب "${escapeHTML(account.name)}" إلى "${escapeHTML(name)}".`);
                account.name = name;
            }
            ...
            logOperation("إضافة حساب", `تم إنشاء حساب جديد "${escapeHTML(name)}" برصيد افتتاحي ${formatCurrency(balance)}.`);
        }
    ...
    accountsList.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('edit-btn')) {
            ...
            if (account) {
                ...
                document.getElementById('account-form-title').textContent = `تعديل حساب: ${escapeHTML(account.name)}`;
        ...
        if (target.classList.contains('delete-btn')) {
            ...
            if (account.balance > 0) {
                 if (!confirm(`تحذير: حساب "${escapeHTML(account.name)}" يحتوي على رصيد (${formatCurrency(account.balance)}).\nهل أنت متأكد من حذفه؟ سيتم فقدان هذا المبلغ.`)) {
                    return;
                }
            } else if (!confirm(`هل أنت متأكد من حذف حساب "${escapeHTML(account.name)}"؟`)){
                 return;
            }
            
            accounts = accounts.filter(acc => acc.id !== id);
            logOperation("حذف حساب", `تم حذف حساب "${escapeHTML(account.name)}".`);
            updateUI();
        }
    });
}
...
if (piItemNameInput) {
    piItemNameInput.addEventListener('change', () => {
        ...
        if (product && piItemCostInput) {
            ...
            showMessage(piItemAddMessage, `تم جلب متوسط التكلفة للمنتج "${escapeHTML(name)}".`, false, true);
        }
    });
}
    ...
    d('liabilities-list-container').addEventListener('click', async (e) => {
        ...
        if (target) {
            ...
            if (invoice) {
                ...
                invoice.items.forEach(item => details += `- ${item.quantity}x ${escapeHTML(item.name)} (${formatCurrency(item.cost)})\n`);
                alert(details);
            } else {
...
function openPartialLiabilityPaymentModal(liabilityId) {
    ...
    accounts.forEach(acc => {
        accountSelect.innerHTML += `<option value="${acc.id}">${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})</option>`;
    });
...
d('confirm-partial-liability-payment-btn').addEventListener('click', () => {
    ...
    if (amountToPay > account.balance) {
        showMessage(partialLiabilityPaymentMessage, `رصيد حساب "${escapeHTML(account.name)}" غير كافٍ.`, true);
        return;
    }
    ...
    let liquidityLogMsg = `تسديد ${formatCurrency(amountToPay)} من التزام "${escapeHTML(liability.name)}" من حساب "${escapeHTML(account.name)}".`;
    if (paymentNote) liquidityLogMsg += ` ملاحظات: ${escapeHTML(paymentNote)}`;
    ...
    let logText = `تسديد ${formatCurrency(amountToPay)} من التزام "${escapeHTML(liability.name)}" من حساب "${escapeHTML(account.name)}".`;
    if (paymentNote) logText += ` ملاحظات: ${escapeHTML(paymentNote)}`;
    if (isExpenseVal) {
        expenses += amountToPay;
        logOperation("تسجيل مصروف", `بقيمة ${formatCurrency(amountToPay)} جنيه - تسديد جزء من التزام "${escapeHTML(liability.name)}" (حساب: "${escapeHTML(account.name)}")`);
    } else {
...
d('confirm-partial-debt-payment-btn').addEventListener('click', () => {
    ...
    const summaryText = `سداد جزئي من ${escapeHTML(debt.name)} (${escapeHTML(debt.reason)})`;
    logOperation("استلام سداد جزئي", `استلام ${formatCurrency(amountToPay)} في حساب "${escapeHTML(account.name)}" من دين "${escapeHTML(debt.name)}".`);
...
window.showPendingSaleDetails = function(pendingId) {
    ...
    if (sale.mainProduct) {
        mainProductName = `${sale.mainProduct.quantity}x ${escapeHTML(sale.mainProduct.name)}`;
    ...
    if (attachments.length > 0) {
        ...
        attachments.forEach(att => {
            ...
            li.innerHTML = `
                <span>${attQty}x ${escapeHTML(att.name)}</span>
...
window.openConvertPendingSaleToDebtDialog = function(pendingSaleId) {
    ...
    (accounts || []).forEach(acc => {
        const option = document.createElement('option');
        option.value = acc.id;
        option.textContent = `${escapeHTML(acc.name)} - الرصيد: ${formatCurrency(Number(acc.balance || 0))}`;
...
debtOptions.forEach(item => {
    ...
    option.textContent = `${escapeHTML(item.name)} - ${formatCurrency(item.total)}`;
...
window.debugShowAllInvoices = function() {
    ...
    salesToday.forEach((inv, index) => {
        ...
        report += `   - العميل: ${escapeHTML(inv.customerName)}\n`;
...
// ==========================================
async function saveSystemToCloud(extraBatchOps = []) {
    ...
    try {
        await FinanceSync.runBatch(async (batch) => {
            // 1. حفظ بيانات اليوم
            const dayRef = window.doc(window.db, "users", userId, "days", dateStr);
            batch.set(dayRef, sanitizedState, { merge: true });
            ...
            if (isStateMeaningful(latestBalancesData)) {
                const summaryRef = window.doc(window.db, "users", userId, "summaries", "latestBalances");
                batch.set(summaryRef, latestBalancesData, { merge: true });
            } else {
                console.log("تم تجاهل تحديث latestBalances لأن البيانات الحالية فارغة أو مصفرة.");
            }

            // 3. العمليات الإضافية (مثل حفظ فاتورة أو مسح مرتجع كعملية ذرية واحدة)
            if (Array.isArray(extraBatchOps)) {
                for (const op of extraBatchOps) {
                    if (op.type === 'set') batch.set(op.ref, op.data, op.opts || { merge: true });
                    else if (op.type === 'update') batch.update(op.ref, op.data);
                    else if (op.type === 'delete') batch.delete(op.ref);
                }
            }
        }, 'saveSystemSnapshotBatch');

        console.log("✅ تمت المزامنة السحابية الذرية (Batch) بنجاح.");
...
function refreshNamesDatalists() {
    ...
    if (debtorDatalist) {
        ...
        debtorDatalist.innerHTML = uniqueNames.map(name => `<option value="${escapeHTML(name)}">`).join('');
    }
    ...
    if (creditorDatalist) {
        ...
        creditorDatalist.innerHTML = uniqueCreditors.map(name => `<option value="${escapeHTML(name)}">`).join('');
    }
}
...
function processTransfer(sourceDebtId) {
    ...
    logOperation("تحويل دين", `تم تحويل ${formatCurrency(transferAmount)} من ${escapeHTML(oldSourceName)} إلى ${escapeHTML(targetProfile.name)}`);
    updateUI();
...
window.paySelectedDebtsForCustomer = function(customerId) {
    ...
    if (confirm(`هل أنت متأكد من سداد ديون بقيمة ${formatCurrency(totalAmount)} وإيداعها في "${escapeHTML(account.name)}"؟`)) {
        // حفظ الحالة للتراجع
        if (typeof saveStateToHistory === 'function') saveStateToHistory();
        ...
        // تسجيل العملية
        logOperation("سداد ديون محددة", `تم سداد ${paidDebtIds.length} بنود ديون للعميل "${escapeHTML(customerName)}" بقيمة ${formatCurrency(totalAmount)} وإيداعها في "${escapeHTML(account.name)}"`);
...
window.openBulkDebtModal = function() {
    ...
    accSelect.innerHTML = accounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})</option>`).join('');
        ...
        header.innerHTML = `
            <div style="display:flex; align-items:center; gap:8px;">
                <label style="cursor:pointer; display:flex; align-items:center; gap:6px; font-weight:bold; color:#1f2937;">
                    ...
                    ${escapeHTML(group.profile.name)} <span style="font-size:12px; color:#6b7280; background:#e5e7eb; padding:2px 6px; border-radius:10px;">${formatCurrency(group.total)}</span>
                </label>
            </div>
        `;
...
window.confirmBulkDebtPayment = function() {
    ...
    if (confirm(`تأكيد سداد ديون بقيمة ${formatCurrency(totalAmount)} لعدد ${involvedCustomers.size} عملاء وإيداعها في "${escapeHTML(account.name)}"؟`)) {
        // حفظ الحالة للتراجع
        if (typeof saveStateToHistory === 'function') saveStateToHistory();
        ...
        // تسجيل اللوج
        let logDesc = `سداد جماعي لعدد ${paidDebtIds.length} بند دين بإجمالي ${formatCurrency(totalAmount)} من العملاء: (${escapeHTML(customerNamesStr)}) وإيداعها في "${escapeHTML(account.name)}"`;
        if (note) logDesc += ` - ملاحظة: ${escapeHTML(note)}`;
...
window.deleteDebtItem = function(debtId) {
    ...
    document.getElementById('delete-debt-info').textContent = `سيتم حذف دين "${escapeHTML(debt.reason)}" بقيمة ${formatCurrency(debt.amount)}. اختر الحساب الذي تريد استرداد المبلغ إليه:`;

    const select = document.getElementById('delete-debt-account-select');
    select.innerHTML = window.accounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (${formatCurrency(acc.balance)})</option>`).join('');
    ...
    document.getElementById('confirmDeleteRefundBtn').onclick = () => {
        ...
        if (account) {
            ...
            logOperation("إلغاء دين", `إعادة ${formatCurrency(debt.amount)} لحساب ${escapeHTML(account.name)} بعد حذف دين ${escapeHTML(debt.name)}`);
            updateUI();
...
window.openUnifiedSellModal = function(originType = 'quick') {
    ...
    if (accountSelect && liveAccounts.length > 0) {
        accountSelect.innerHTML = liveAccounts.map(acc => `<option value="${acc.id}">${escapeHTML(acc.name)} (${acc.balance.toFixed(2)} جنيه)</option>`).join('');
    }
