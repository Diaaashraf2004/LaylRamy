            ...
            <div class="exchange-card">
                ...
                <div class="delivery-status">
                    ...
                    <div class="radio-group" style="margin-top:10px;">
                        ...
                        <label for="del_now" style="margin-left: 15px;">📦 استبدال فوري (العميل في المحل وتم التبادل)</label>
                        
                        <input type="radio" id="del_later" name="delivery_type" value="pending" onchange="updateUIState()">
                        <label for="del_later">⏳ شحن للعميل (تعليق البضاعة/الفلوس حتى التسليم)</label>
                    </div>
                    <div id="pending_payment_options" style="display:none; margin-top:15px; padding:10px; background:#ebf8ff; border-radius:6px; border:1px solid #90cdf4;">
                        <p style="margin:0 0 10px 0; font-weight:bold; color:#2b6cb0;">خيارات الدفع للشحن:</p>
                        <div style="display:flex; flex-direction:column; gap:8px;">
                            <label><input type="radio" name="pending_pay_type" value="pay_later" checked> تأجيل الدفع (استلام/دفع الفلوس وقت تسليم البضاعة)</label>
                            <label><input type="radio" name="pending_pay_type" value="pay_now_safe"> تم استلام/دفع الفلوس الآن (تسجيل في الخزنة المحددة)</label>
                            <label><input type="radio" name="pending_pay_type" value="pay_now_debt"> تسجيل فرق الحساب كـ "دين" على العميل</label>
                        </div>
                    </div>
                    <div id="shipping_info" style="display:none; margin-top:15px; padding:10px; background:#f0fff4; border-radius:6px; border:1px solid #9ae6b4;">
                        <p style="margin:0 0 10px 0; font-weight:bold; color:#276749;">بيانات الشحن:</p>
                        <div class="grid-2" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                            <div><label>شركة الشحن</label><input type="text" id="ex_shipping_company" class="form-control" placeholder="مثال: ارامكس"></div>
                            <div><label>رقم التتبع / الشحنة</label><input type="text" id="ex_tracking_number" class="form-control" placeholder="رقم بوليصة الشحن"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="customer-details" style="margin-bottom: 20px; padding: 15px; background: #f8fafc; border: 1px solid #cbd5e0; border-radius: 8px;">
                    ...
                    <button type="button" onclick="lookupCustomerInvoices()" style="background:#2b6cb0; color:#fff; border:none; padding:6px 12px; border-radius:4px; font-size:12px; cursor:pointer; margin-top:8px;">🔍 استدعاء فواتير العميل</button>
                </div>
...
function renderLocalPendingOrders() {
    ...
    if (!window.pendingOrders || window.pendingOrders.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#a0aec0;">لا توجد فواتير معلقة حالياً.</td></tr>`;
        return;
    }
    tbody.innerHTML = window.pendingOrders.map(o => {
        const hasStages = !!o.stages;
        const shippingStr = (o.shippingCompany || o.trackingNumber) ? `${o.shippingCompany||'-'} <br> ${o.trackingNumber||'-'}` : '-';
        let stageCol = '-';
        let actionCol = '';

        if (hasStages) {
            const s1 = o.stages.newItemShipped ? '🟢' : '🔴';
            const s2 = o.stages.moneyCollected ? '🟢' : '🔴';
            const s3 = o.stages.oldItemReceived ? '🟢' : '🔴';
            stageCol = `<span title="شحن الجديد">${s1}</span> <span title="التحصيل">${s2}</span> <span title="استلام المرتجع">${s3}</span>`;
            actionCol = `<button class="btn-action" style="background:#4299e1;" onclick="openStagesModal('${o.id}')">⚙️ إدارة المراحل</button>
            ...
            actionCol = `<button class="btn-action btn-confirm" onclick="confirmPendingOrder('${o.id}')">✔️ تحصيل</button>
                         <button class="btn-action btn-edit" onclick="openExModalEdit('${o.id}')">✏️ تعديل</button>
                         <button class="btn-action btn-cancel" onclick="openExModalCancel('${o.id}')">❌ إلغاء</button>`;
        }
        ...
        return `
            <tr>
                ...
                <td style="font-size:12px; text-align:center;">${shippingStr}</td>
                ...
                <td style="font-size:16px; letter-spacing:3px;">${stageCol}</td>
                <td>${actionCol}</td>
            </tr>
        `;
    }).join('');
...
function updateUIState() {
    ...
    const payOpts = document.getElementById('pending_payment_options');
    const shipInfo = document.getElementById('shipping_info');
    if(payOpts) payOpts.style.display = isPending ? 'block' : 'none';
    if(shipInfo) shipInfo.style.display = isPending ? 'block' : 'none';

    if(isPending) {
        btn.style.background = "#ecc94b"; btn.style.color = "#1a202c"; btn.innerHTML = "حفظ كطلب معلق ⏳";
    } else {
...
async function executeTransaction() {
    ...
    const pendingPayType = isPending ? (document.querySelector('input[name="pending_pay_type"]:checked')?.value || 'pay_later') : null;
    const shippingCompany = document.getElementById('ex_shipping_company')?.value || "";
    const trackingNumber = document.getElementById('ex_tracking_number')?.value || "";
    ...
    try {
     ...
     const processReturn = (name, cost, price, cat) => {
        ...
        if (isPending && itemsToSell.length > 0) {
            const stages = {
                newItemShipped: false,
                newItemShippedDate: null,
                moneyCollected: pendingPayType !== 'pay_later',
                moneyCollectedDate: pendingPayType !== 'pay_later' ? new Date().toISOString() : null,
                moneyMethod: pendingPayType,
                oldItemReceived: false,
                oldItemReceivedDate: null
            };
            ...
            if (pendingPayType === 'pay_now_safe') {
                const liveAccounts = getAccounts();
                const acc = liveAccounts.find(a => a.id === accId);
                if(acc) acc.balance = (Number(acc.balance)||0) + diff;
                addLogSafe({ timestamp: new Date().toISOString(), type: "دفع آجل لخزنة", details: `تسجيل فرق الاستبدال للعميل [${custNameInput}] في الخزنة`, amount: diff });
            } else if (pendingPayType === 'pay_now_debt') {
                window.debtors = window.debtors || [];
                let debtor = window.debtors.find(d => d.name === custNameInput || d.customerName === custNameInput);
                if (!debtor) {
                    debtor = { id: "DEBT-"+Date.now(), name: custNameInput, customerName: custNameInput, phone: custPhone, amount: 0, reason: "دين استبدال مؤجل", createdAt: new Date().toISOString() };
                    window.debtors.push(debtor);
                }
                debtor.amount = (Number(debtor.amount)||0) + diff;
                addLogSafe({ timestamp: new Date().toISOString(), type: "دين استبدال", details: `تسجيل فرق الاستبدال كدين على [${custNameInput}]`, amount: diff });
            } else {
...
// =======================
// مراحل الطلبات المعلقة والفواتير
// =======================

window.openStagesModal = function(id) {
    const o = window.pendingOrders.find(x => x.id === id);
    if(!o || !o.stages) return alert("لا توجد مراحل لهذا الطلب");
    
    let modal = document.getElementById('ex-stages-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'ex-stages-modal';
    ...
    let html = `
    <div style="background:#fff; width:90%; max-width:500px; border-radius:8px; padding:20px; text-align:right;" dir="rtl">
        <h3 style="margin-top:0; color:#2b6cb0;">إدارة مراحل الطلب: ${o.customerName}</h3>
        
        <div style="margin-bottom:15px; padding:10px; border:1px solid #e2e8f0; border-radius:6px; background:${o.stages.newItemShipped ? '#f0fff4' : '#fff'}">
            <h4 style="margin:0 0 10px 0;">1. شحن البضاعة الجديدة ${o.stages.newItemShipped ? '✅' : '⏳'}</h4>
            <p style="margin:0 0 10px 0; font-size:14px; color:#4a5568;">المنتجات: ${o.nMainName} ${o.nAccs && o.nAccs.length ? '+ '+o.nAccs.length+' إضافات' : ''}</p>
            ${!o.stages.newItemShipped ? `<button onclick="confirmStage('${id}', 'newItemShipped')" style="background:#38a169; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">تأكيد خروج البضاعة</button>` : ''}
        </div>

        <div style="margin-bottom:15px; padding:10px; border:1px solid #e2e8f0; border-radius:6px; background:${o.stages.moneyCollected ? '#f0fff4' : '#fff'}">
            <h4 style="margin:0 0 10px 0;">2. تحصيل الفلوس (${o.diffAmount} ج.م) ${o.stages.moneyCollected ? '✅' : '⏳'}</h4>
            ${!o.stages.moneyCollected ? `
                <select id="stage_pay_method" style="margin-bottom:10px; width:100%; padding:5px;">
                    <option value="pay_now_safe">تسجيل في الخزنة (${o.accId})</option>
                    <option value="pay_now_debt">تسجيل كدين على العميل</option>
                </select>
                <button onclick="confirmStage('${id}', 'moneyCollected')" style="background:#38a169; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">تأكيد التحصيل</button>
            ` : ''}
        </div>

        <div style="margin-bottom:15px; padding:10px; border:1px solid #e2e8f0; border-radius:6px; background:${o.stages.oldItemReceived ? '#f0fff4' : '#fff'}">
            <h4 style="margin:0 0 10px 0;">3. استلام المرتجع ${o.stages.oldItemReceived ? '✅' : '⏳'}</h4>
            <p style="margin:0 0 10px 0; font-size:14px; color:#4a5568;">المنتجات: ${o.rMainName} ${o.rAccs && o.rAccs.length ? '+ '+o.rAccs.length+' إضافات' : ''}</p>
            ${!o.stages.oldItemReceived ? `<button onclick="confirmStage('${id}', 'oldItemReceived')" style="background:#38a169; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">تأكيد استلام المرتجع</button>` : ''}
        </div>

        <div style="text-align:left; margin-top:20px;">
            <button onclick="document.getElementById('ex-stages-modal').style.display='none'" style="background:#e53e3e; color:#fff; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">إغلاق</button>
        </div>
    </div>
    `;
...
window.confirmStage = async function(id, stageName) {
    const o = window.pendingOrders.find(x => x.id === id);
    if(!o || !o.stages) return;

    if(stageName === 'newItemShipped') {
        const liveProducts = getProducts();
        const processOut = (name, cost, price) => {
            if(!name) return;
            const cleanName = String(name).trim().toLowerCase();
        ...
        o.stages.newItemShipped = true;
        o.stages.newItemShippedDate = new Date().toISOString();
        addLogSafe({ timestamp: new Date().toISOString(), type: "شحن بضاعة", details: `تم شحن البضاعة للعميل [${o.customerName}]`, amount: 0 });
    }
    else if(stageName === 'moneyCollected') {
        const method = document.getElementById('stage_pay_method').value;
        if (method === 'pay_now_safe') {
            const liveAccounts = getAccounts();
            const acc = liveAccounts.find(a => a.id === o.accId);
            if(acc) acc.balance = (Number(acc.balance)||0) + o.diffAmount;
            addLogSafe({ timestamp: new Date().toISOString(), type: "تحصيل آجل لخزنة", details: `تحصيل فرق الاستبدال للعميل [${o.customerName}] في الخزنة`, amount: o.diffAmount });
        } else if (method === 'pay_now_debt') {
            window.debtors = window.debtors || [];
            let debtor = window.debtors.find(d => d.name === o.customerName || d.customerName === o.customerName);
            if (!debtor) {
                debtor = { id: "DEBT-"+Date.now(), name: o.customerName, customerName: o.customerName, phone: o.customerPhone, amount: 0, reason: "دين استبدال", createdAt: new Date().toISOString() };
                window.debtors.push(debtor);
            }
            debtor.amount = (Number(debtor.amount)||0) + o.diffAmount;
            addLogSafe({ timestamp: new Date().toISOString(), type: "دين استبدال", details: `تسجيل فرق الاستبدال كدين على [${o.customerName}]`, amount: o.diffAmount });
        }
        
        o.stages.moneyCollected = true;
        o.stages.moneyCollectedDate = new Date().toISOString();
        o.stages.moneyMethod = method;
    }
    else if(stageName === 'oldItemReceived') {
        const liveProducts = getProducts();
        ...
        o.stages.oldItemReceived = true;
        o.stages.oldItemReceivedDate = new Date().toISOString();
        addLogSafe({ timestamp: new Date().toISOString(), type: "استلام مرتجع", details: `تم استلام البضاعة المرتجعة من العميل [${o.customerName}]`, amount: 0 });
    }

    if (o.stages.newItemShipped && o.stages.moneyCollected && o.stages.oldItemReceived) {
        const orderIndex = window.pendingOrders.findIndex(x => x.id === id);
        if(orderIndex !== -1) window.pendingOrders.splice(orderIndex, 1);
        document.getElementById('ex-stages-modal').style.display='none';
    } else {
        openStagesModal(id);
    }
...
window.lookupCustomerInvoices = async function() {
    const custName = document.getElementById('ex_customer_name').value.trim();
    if(!custName) return alert('يرجى إدخال اسم العميل أولاً للبحث عن فواتيره');
    
    let results = [];
    const lowerName = custName.toLowerCase();
    
    if(window.salesToday) {
        results = results.concat(window.salesToday.filter(s => s.customerName && s.customerName.toLowerCase().includes(lowerName)));
    }
    if(window.invoices) {
        results = results.concat(window.invoices.filter(s => s.customerName && s.customerName.toLowerCase().includes(lowerName)));
    }
    ...
    let modal = document.getElementById('ex-invoice-lookup-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'ex-invoice-lookup-modal';
        modal.className = 'modal';
        modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999; display:none; justify-content:center; align-items:center;';
        document.body.appendChild(modal);
    }
    ...
    let tbody = results.length === 0 ? `<tr><td colspan="4" style="text-align:center;">لا توجد فواتير مطابقة</td></tr>` :
        results.map(s => `
            <tr>
                ...
                <td><button onclick='selectInvoiceForReturn(${JSON.stringify(s).replace(/'/g, "&#39;")})' style="background:#3182ce; color:#fff; border:none; padding:4px 8px; border-radius:4px; cursor:pointer;">اختيار</button></td>
            </tr>
        `).join('');

    let html = `
    <div style="background:#fff; width:90%; max-width:600px; max-height:80vh; overflow-y:auto; border-radius:8px; padding:20px; text-align:right;" dir="rtl">
        <h3 style="margin-top:0; color:#2b6cb0;">فواتير العميل: ${custName}</h3>
        <table style="width:100%; border-collapse:collapse; margin-bottom:20px;">
            <thead>
                <tr style="background:#edf2f7;">
                    <th style="padding:10px; border-bottom:2px solid #cbd5e0;">رقم الفاتورة</th>
                    <th style="padding:10px; border-bottom:2px solid #cbd5e0;">التاريخ</th>
                    <th style="padding:10px; border-bottom:2px solid #cbd5e0;">المنتج</th>
                    <th style="padding:10px; border-bottom:2px solid #cbd5e0;">إجراء</th>
                </tr>
            </thead>
            <tbody>${tbody}</tbody>
        </table>
        <div style="text-align:left;">
            <button onclick="document.getElementById('ex-invoice-lookup-modal').style.display='none'" style="background:#e53e3e; color:#fff; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;">إغلاق</button>
        </div>
    </div>
    `;
    modal.innerHTML = html;
...
window.selectInvoiceForReturn = function(invoice) {
    if(!invoice || !invoice.items || invoice.items.length === 0) {
        document.getElementById('r_main_name').value = invoice.productName || invoice.name || '';
        document.getElementById('r_main_cost').value = invoice.cost || invoice.costPrice || 0;
        document.getElementById('r_main_price').value = invoice.sellPrice || invoice.price || 0;
    } else {
        document.getElementById('r_main_name').value = invoice.items[0].productName || invoice.items[0].name || '';
        document.getElementById('r_main_cost').value = invoice.items[0].cost || invoice.items[0].costPrice || 0;
        document.getElementById('r_main_price').value = invoice.items[0].price || invoice.items[0].unitPrice || 0;
        
        const accCont = document.getElementById('r_acc_container');
        if(accCont) accCont.innerHTML = '';
        
        for(let i=1; i<invoice.items.length; i++) {
            if (typeof addAccRow === 'function') {
                addAccRow('r');
                const rows = accCont.querySelectorAll('.r-acc-row');
                const newRow = rows[rows.length - 1];
                if(newRow) {
                    const nInput = newRow.querySelector('.acc-name');
                    const cInput = newRow.querySelector('.acc-cost');
                    const pInput = newRow.querySelector('.acc-price');
                    if(nInput) nInput.value = invoice.items[i].productName || invoice.items[i].name || '';
                    if(cInput) cInput.value = invoice.items[i].cost || invoice.items[i].costPrice || 0;
                    if(pInput) pInput.value = invoice.items[i].price || invoice.items[i].unitPrice || 0;
                }
            }
        }
    }
